...
function randomForest(X,y,depth,k)
	(n,d)=size(X)
	#select n sample using bootstrapping from all X
bootstrap=zeros(k,n)
subModels=Array{GenericModel}(undef,k)
 for i in 1:k
	# k: #training set
	bootstrap[i,:]=rand(1:n,n)
    media=bootstrap[i,:].|>Int
	#randomTree_sub(X[media,:],y[media],depth)
subModels[i]=randomTree_sub(X[media,:],y[media],depth)
 end
